...
while True:
    ...
    avg_queue_time = seconds_elapsed
    ...
    cv2.putText(frame, yellow_count_text, (10, image_height - 50), FONT_FACE, FONT_SCALE, WHITE, 1, cv2.LINE_AA)

    # Exibir a média dos tempos de espera no canto inferior esquerdo
    if waiting_people_count > 0:
        avg_time_per_person = avg_queue_time / waiting_people_count
    else:
        avg_time_per_person = 0

    avg_time_text = f"Media do tempo em fila: {avg_time_per_person:.1f}s"
